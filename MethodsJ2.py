...
def main():
    ...
    #
    ###################### Still need to implement java-based version, as discussed with Thomas...
    #
    ############## retrieve materials and methods map/json file

    # page_to_retrieve = urllib2.urlopen('https://raw.githubusercontent.com/joelryan/variousSandboxStuff/master/acquisitionSettings_textGen_v07.json')
    ...
    def welcomeBox():
        ...
        return [filename, componentsJson, showBioFormatsMetadata, showOrigMetadata]
    ...
    def dialog_getter(settings, i):
        if settings[i].get('Dialog_Type') == 'addCheckbox':
            ...
            print_and_log(settings[i].get('Setting') + (settings[i]['userInput']))
    ...
    microscope_stand = data['MicroscopeStand']
    ### get all components in microscope from json file
    components = data['components']
	...
	gui = GenericDialogPlus("Acknowledgements")
    for i in range(0, len(settings)):
        if settings[i].get('Dialog_Box') == "Acknowledgements":
            dialog_adder(settings, i)
    gui.showDialog()
    if gui.wasCanceled():
        return None
    ...
    for i in range(0, len(settings)):
        if settings[i].get('Dialog_Box') == "Acknowledgements":
            user_input = str(dialog_getter(settings, i))
            acknowledgement_blurb += ' ' + getInfoAndBlurb(user_input, settings, i)
    ...
    showText(blurb, "MethodsJ2 output", "MethodsJ2 text generation based on user input and on a Micro-Meta App hardware file")
